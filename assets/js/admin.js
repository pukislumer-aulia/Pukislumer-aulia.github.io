...
const savePromoBtn = document.getElementById("savePromo");
    ...
    orders.forEach(o => {
        tbody.innerHTML += `
            <tr>
                ...
                <td>${o.wa}</td>
                <td>${o.jenis} (${o.isi})</td>
                <td>${o.toppingList ? o.toppingList.join(", ") : "-"}</td>
                <td>${o.jumlahBox} box</td>
                <td>Rp${Number(o.total).toLocaleString()}</td>
                <td>${new Date(o.createdAt).toLocaleString()}</td>
                <td>${o.invoice}</td>
                <td><button onclick="reprintInvoice('${o.invoice}')">Cetak Ulang</button></td>
            </tr>`;
    });
}
...
function reprintInvoice(inv) {
    ...
    generatePDF(data, true); // Pastikan fungsi generatePDF tersedia
}
...
document.getElementById("exportCSV")?.addEventListener("click", () => {
    ...
    let csv = "invoice,nama,wa,jenis,isi,topping,taburan,jumlah,total,queue,lunas,tanggal\n" +
        orders.map(o => `
            ...
            ${o.jenis},
            ${o.isi},
            "${o.toppingList ? o.toppingList.join(";") : ""}",
            "${o.taburanList ? o.taburanList.join(";") : ""}",
            ${o.jumlahBox},
            ${o.total},
            ${o.queue},
            ${o.lunas},
