...
// Final unified version with robust PDF & clean table columns
// ===============================
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateToppingDisplay() {
    ...
    if (mode === "non") $$(".ultraTopping, .ultraTaburan").forEach(x => x.checked = false);
    calculatePrice();
  }

  $$('input[name="ultraToppingMode"]').forEach(r => r.addEventListener("change", updateToppingDisplay));
  try { updateToppingDisplay(); } catch {}
  ...
  function toppingLimitEvent(e) {
    ...
    if ((mode === "single" && t.length > 5) ||
        (mode === "double" && ((e.target.classList.contains("ultraTopping") && t.length > 5) || (e.target.classList.contains("ultraTaburan") && tb.length > 5)))) {
      e.target.checked = false;
      alert("Maksimal 5 topping/taburan!");
      return;
    }
    calculatePrice();
  }
  ...
  function calculatePrice() {
    ...
    const pricePerBox = (BASE_PRICE[jenis] && BASE_PRICE[jenis][isi] && BASE_PRICE[jenis][isi][mode]) || 0;
  ...
  notaSendAdmin.addEventListener("click", () => {
    ...
    if (d.mode === "single") msg += `Topping: ${d.topping.join(", ") || "-"}\n`;
    if (d.mode === "double") msg += `Topping: ${d.topping.join(", ") || "-"}\nTaburan: ${d.taburan.join(", ") || "-"}\n`;
    msg += `Jumlah: ${d.jumlahBox} Box\nTotal: ${formatRp(d.total)}\nCatatan: ${d.note}\n\nTerimakasih 🙏`;
  ...
  notaPrint.addEventListener("click", async () => {
    try {
      ...
      console.error("PDF gagal dibuat:", err);
      alert("PDF gagal dibuat: " + err.message);
    }
  });
...
async function generatePdf(data) {
  ...
  const {
    nama = "-", jenis = "-", isi = "-", mode = "non",
    topping = [], taburan = [], jumlahBox = 1,
    pricePerBox = 0, subtotal = 0, discount = 0, total = 0,
    note = "-", orderID = "ORD-" + Date.now()
  } = data;
  ...
  try { pdf.setFont("Pacifico-Regular"); } catch {}
  pdf.setFontSize(26);
  ...
  if (note && note!=="-") { y+=10; pdf.text("Catatan :",10,y); pdf.text(note,10,y+6); y+=12; } else y+=10;
  ...
  pdf.setFont("helvetica","bold");
  pdf.text("Total Bayar: "+fmtRp(total),195,finalY+(discount>0?12:6),{align:"right"});
