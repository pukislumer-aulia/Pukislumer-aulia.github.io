...
// Pastikan jsPDF & jsPDF-Autotable dimuat via <script> di HTML, tidak perlu import di sini
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const getSelectedRadioValue = name => document.querySelector(`input[name="${name}"]:checked`)?.value || null;
  const getCheckedValues = selector => Array.from(document.querySelectorAll(selector)).filter(ch=>ch.checked).map(ch=>ch.value);

  /* ================= LIMIT CHECKBOXES ================= */
  const enforceMax = (selector,max)=>{
    $$(selector).forEach(cb=>{
      cb.addEventListener("change",()=>{
        const checkedCount = $$(selector).filter(x=>x.checked).length;
        if(checkedCount>max){ cb.checked=false; alert(`Maksimal ${max} pilihan.`); }
      });
    });
  };
  ...
  const calculatePrice = ()=>{
    ...
    const mode = getSelectedRadioValue("ultraToppingMode") || "non";
    const basePerBox = (BASE_PRICE[jenis]?.[isi] || BASE_PRICE["Original"][5]) || 0;
  ...
  const generateInvoicePDF = order=>{
    ...
    if(!jsPDF){ alert("jsPDF tidak tersedia."); return null; }
    const doc = new jsPDF();
...
`;
  ...
  const openWA = order=> window.open(`https://wa.me/${ADMIN_WA}?text=${encodeURIComponent(buildWAMessage(order))}`,"_blank");
