...
(function () {
  ...
  function buildToppingsUI() {
    ...
    if (elDoubleGroup) {
      ...
      DOUBLE_TABURAN.forEach(t => {
        ...
        const input = document.createElement('input');
        input.type = 'checkbox';
        input.name = 'taburan'; // allow multiple taburan; order.js treats existence as +hargaTaburan per box
        input.value = t;
        input.id = id;
        label.appendChild(input);
        label.appendChild(document.createTextNode(' ' + t));
  ...
  function updateToppingVisibility() {
    const mode = getRadioValue('ultraToppingMode') || (document.querySelector('input[name="ultraToppingMode"]:checked') && document.querySelector('input[name="ultraToppingMode"]:checked').value) || 'non';
    // default hide both
  ...
  function getPricePerBox() {
    ...
    const jenis = String(jenisRaw).toLowerCase(); // 'Original' or 'Pandan' expected by BASE_PRICE keys with capitalization; we'll normalize
    // Normalize to match BASE_PRICE keys
    const jenisKey = (jenis.toLowerCase() === 'original') ? 'Original' : (jenis.toLowerCase() === 'pandan' ? 'Pandan' : null);
    ...
    // map mode to the key names in priceObj: non/single/double
    const price = priceObj[mode] || priceObj['non'] || 0;
    return price;
  }

  // Discount logic (preserve earlier behavior)
  ...
  function validateOrder() {
    ...
    // if mode single => ensure a 'topping' radio is selected (we used name="topping" for singles)
    if (mode === 'single') {
      const s = getRadioValue('topping'); // single toppings are radio named 'topping'
      if (!s) { alert('Pilih topping single'); return false; }
    }

    // if mode double => ensure two radio groups topping_double_1 and topping_double_2 selected and different
    if (mode === 'double') {
    ...
    // taburan: if any taburan checkboxes exist and are visible, require at least one? 
    // Original behavior allowed optional; here we keep optional (if present chosen, add price)
    return true;
  }

  // ----------------------------
  // Build order object (to display nota & save)
  // ----------------------------
  function buildOrderObject() {
    ...
    let doubleChosen = [];
    if (mode === 'single') {
      const s = getRadioValue('topping'); // name='topping'
      if (s) singleChosen = [s];
    } else if (mode === 'double') {
      const t1 = getRadioValue('topping_double_1');
      const t2 = getRadioValue('topping_double_2');
  ...
  function renderNota(order) {
    ...
    lines.push(`<div><strong>Jumlah Box:</strong> ${order.jumlah || (order.jumlah===0?0: '')}${order.jumlah? ' box' : ''}</div>`);
    lines.push('<hr>');
    if (order.mode === 'single') {
      lines.push(`<div><strong>Topping:</strong> ${escapeHtml((order.single && order.single[0]) || '-')}</div>`);
    ...
    lines.push(`<div><strong>Taburan:</strong> ${escapeHtml((order.taburan && order.taburan.join(', ')) || '-')}</div>`);
    lines.push('<hr>');
  ...
  // ----------------------------
  // Event: cek pesanan (open nota)
  // ----------------------------
  btnCek.addEventListener('click', function (ev) {
    ev.preventDefault();
    // validate and build order object for preview
    const order = buildOrderObject();
    if (!order) return;
    // render nota and show
    const html = renderNota(order);
    showNotaOverlay(html);
  });

  // ----------------------------
  // Event: konfirmasi buat pesanan (di dalam nota)
  // ----------------------------
  if (notaConfirm) {
  ...
  // Listen to changes for price recalculation
  // Watch relevant inputs: jenis radio (name=ultraJenis), isi select, mode topping radio, topping groups, taburan checkboxes, jumlah
  function attachChangeListeners() {
    // jenis radios
    const jenisInputs = document.querySelectorAll('input[name="ultraJenis"]');
    jenisInputs.forEach(i => i.addEventListener('change', function () {
      updatePriceUI();
    }));

    // isi select
    const isiEl = $('#ultraIsi');
    if (isiEl) isiEl.addEventListener('change', function () {
      updatePriceUI();
    });
    ...
    // jumlah
    if (elJumlah) elJumlah.addEventListener('input', function () {
      updatePriceUI();
    });

    // delegate events for dynamically created topping/taburan inputs
    document.addEventListener('change', function (e) {
      // topping single (name='topping')
      if (e.target && e.target.name === 'topping') {
        updatePriceUI();
      }
      // double toppings
      if (e.target && (e.target.name === 'topping_double_1' || e.target.name === 'topping_double_2')) {
        updatePriceUI();
      }
      // taburan
      if (e.target && e.target.name === 'taburan') {
        updatePriceUI();
      }
    });
  }
  ...
  // Expose some helpers (for debugging or integration) without polluting globals heavily
  window._pukis = {
    buildToppingsUI,
    updateToppingVisibility,
    updatePriceUI,
